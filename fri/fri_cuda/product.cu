...
int main(int argc, char** argv) {
    ...
    generateWithoutInv<<<gridSize, blockSize, 0, stream_gen>>>(d_twiddles_ntt, generator, n, p);
    ...
    ntt(d_coeff, d_twiddles_ntt, n, p, stream_main);

    FRICommitmentGPU fri = fri_commitment_gpu(d_coeff, n, log_n, stream_main);
