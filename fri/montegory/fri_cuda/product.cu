...
int main(int argc, char** argv) {
    ...
    // ── 병렬 Setup ────────────────────────────────────────────
    // stream_gen: twiddle 생성 (compute 엔진)
    generateWithoutInv<<<gridSize, blockSize, 0, stream_gen>>>(d_twiddles_ntt, g_m, n);
    cudaEventRecord(gen_done, stream_gen);

    // stream_transfer: H2D 전송 (PCIe copy 엔진)
    ...
    // stream_main: 두 stream 완료 후 시작 (CPU 블록킹 없음)
